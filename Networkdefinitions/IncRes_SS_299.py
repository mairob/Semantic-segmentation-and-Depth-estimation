def inference(images, isTrainingRun = True, isTrainingFromScratch=True):
  ...
  with tf.name_scope('NEUTRAL_Branch'):
    with tf.variable_scope('NEUTRAL_Branch'):
      ...
      n_up_5 = LayerBlocks.up_project(n_up_4, size=[3, 3, 56, 20], id = 'n_2x_e', stride = 1, training=isTrainingRun) #16
  ...
  with tf.name_scope('SEMANCTIC_Branch'):
    with tf.variable_scope('SEMANCTIC_Branch'):
      ...
      dil_s_up_4 = tf.contrib.layers.conv2d(s_up_4, 32, kernel_size=3, rate=32)
...
def loss_fn(logits, annotation_batch):
  ...
  with tf.variable_scope("Loss"):
    ...
    print("XEntro_Logits : " + str(logits.get_shape()[:]))
    print("XEntro_Annotations : " + str(annotation_batch.get_shape()[:]))
